...
<div class='inside'>
	...
	<div class="col" style="vertical-align:top;">
		<form class='form' id="formcambiarusuario" action='accounts/doEdit' method='POST' enctype='multipart/form-data'>
			<h3 id="none"><?= $CANVIAR_NOMBRE ?></h3>
			<div class='control-group'><label class='control-label'><?= $NUEVO_NOMBRE ?>:<div class="vermell">*</div></label>
				<div class='controls'>
					<input id="min" type='text' name='username' value='<?= $items['username'] ?>'>
				</div>
			</div>
			<input type="hidden" name="accountsId" value="<?= $_SESSION["accountId"]; ?>">
			<input type="button" href="#modalcambiarusuario" class="button black" value="<?= $GUARDAR_T ?>" onclick="doForm(this.form);">
		</div>
	</form>
</div>
