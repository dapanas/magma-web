...
<div id="publicaciones" class="inside">
	...
	<div>
		<div id="table1">
			...
			<table>
				<thead>
					<tr><th><?= $date ?></th><th><?= $TITULO ?></th><th><?= $ESTADO ?></th></tr>
				</thead>
				<tbody>
					<? foreach($items_ahora as $item): ?>
					<tr>
						<td><?= mysql_to_fecha($item['fecha_inicio']) ?></td>
						<td><a href="eventos/detalle/<?=$item['id']?>"><?= $item['titulo_'.$lang] ?></a></td>
						<td><?= $item['cancelado'] == "1" ? $CANCELADO : ($item['publicado'] == "1" ? $PUBLICADO : $PENDIENTE) ?></td>
					</tr>
				<? endforeach; ?>
				</tbody>
			</table>
