<? include('views/includes/top.php'); ?>
<div id="ayuda" >
<img src="views/img/fondo-la-app.png" id="la-app-fondo">

<div id="promo">

<?= $ANUNCIA  ?></div>
<div class="inside" style="width:420px;">
   <p style="font-family: 'FreeSet-DemiBold';"><?= $PUBLICIDAD_SEGMENTADA ?></p>
<p style="font-family: 'FreeSet-DemiBold';"><?= $PRECISAMENTE ?></p>
  
  <h2><?= $COMO ?></h2>
  <p><?= $COMO_TEXT ?></p>
  <img src="views/img/foto.png">
  <br>
  <p><a href="#" style="color: #404040;"><?= $TARIFAS ?></a></p>
  <h2><?= $PERQUE_A_MAGMA ?></h2>
  <p><?= $TE_RESUMIMOS ?></p>
  
  <p style="margin: 0px;font-family: 'FreeSet-DemiBold';"><?= $PEL_QUE_FA ?></p>
  <ul>
<li><?= $PRIMERA_AGENDA ?></li>
<li><?= $DESCARGA_GRATIS ?></li>
<li><?= $PUBLI ?></li>
<li><?= $CONTENIDO ?></li>
<li><?= $INFORMACION_DISPONIBLE ?></li>
<li><?= $COMPARTIR_EVENTOS ?></li>
<li><?= $PERMITE_NOTIFICAR ?></li>
<li><?= $PRESENTA_DISENYO ?></li>
<li><?= $CREA_EXPERIENCIA ?></li>
  </ul>
  
  
<p style="margin: 0px;font-family: 'FreeSet-DemiBold';"><?= $MERCADO ?></p>
<ul>
<li><?= $AMPLIA_COBERTURA ?></li>
<li><?= $PERMITE_CAPACIDAD  ?></li>
  </ul>
</div>
</div>
<? include('views/includes/footer.php'); ?>